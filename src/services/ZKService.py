...
class ZKServices:
    ...
    def obtener_asistencias(
        ...
        try:
            ...
            if rango_fechas:
                if isinstance(rango_fechas, list):
                    if len(rango_fechas) != 2:
                        raise ValueError(
                            "El rango de fechas debe contener exactamente 2 elementos"
                        )
                    rango_fechas = (
            ...
            for asist in asistencias:
                ...
                empleado = (
                    list(filter(lambda e: str(e.uid) == str(asist.user_id), users))
                    or None
                )
    ...
    def verificar_puntualidad(self, id_empleado: int):
        ...
        if not asistencias:
            return {
                "success": False,
                "message": "No hay asistencia registrada para hoy.",
            }
        ...
        if hora_asistencia <= hora_limite:
            return {
                "success": True,
                "message": f" Llegaste a tiempo: {hora_asistencia}",
            }
        else:
