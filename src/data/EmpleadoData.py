...
class EmpleadoData:
    ...
    def create_Empleado(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if id_rol:
                result = self.emplRolData.create_rol_empleado(
                    id_empleado, id_rol, conexion
                )
                if not result["success"]:
                    conexion.rollback()
                    return result
            ...
            if usuario:
                ...
                if id_per:
                    result = self.userPerfilData.create_usuario_perfil(
                        id_usuario, id_per, conexion
                    )
                    if not result["success"]:
                        conexion.rollback()
                        return result
            ...
            return {
                "success": False,
                "message": f"Ocurrió un error al registrar el empleado",
            }
        finally:
            if conexion:
                conexion.close()
    ...
    def registrar_empleado(
        self, id_persona: int, id_departamento: int, conexionEx=None
    ) -> Dict[str, Any]:
        conexion, resultado = (
    ...
    def update_Empleado(self, id_empledo: int, datos: Dict[str, Any]):
        ...
        try:
            ...
            id_per: int = datos.get(
                "id_perfil"
            )  # obtener por por usuario para la relacion usuario perfil
            ...
            with conexion.cursor() as cursor:
                ...
                if listaTel:
                    for telefono in listaTel:
                        ...
                        if telefono.id:  # actualiza
                            result = self.telefonoData.update_telefono(
                                telefono, conexion
                            )
                            if not result["success"]:
                                conexion.rollback()
                            ...
                            result = self.telefonoData.create_telefono(
                                telefono, conexion
                            )
                            if not result["success"]:
                                conexion.rollback()
                ...
                result = self.emplRolData.get_roles_empleado_by_id_empleado(
                    id_empledo, conexion
                )
                if not result["success"]:
                    conexion.rollback()
                ...
                if result["exists"] and id_rol is None:
                    ...
                    resultAux = self.emplRolData.delete_rol_empleado(
                        result["rolesEmpleado"]["id"], conexion
                    )
                ...
                # si existe rolEmpleado y id_rol no es None se procede a actualizar
                    ...
                    resultAux = self.emplRolData.update_rol_empleado(
                        result["rolesEmpleado"]["id"], id_empledo, id_rol, conexion
                    )
                    if not resultAux["success"]:
                        conexion.rollback()
                        return resultAux
                # si no existe rolEmpleado y id_rol no es None se procede a crear
                    ...
                    # crear rolEmpleado
                    resultAux = self.emplRolData.create_rol_empleado(
                        id_empledo, id_rol, conexion
                    )
                    if not resultAux["success"]:
                        conexion.rollback()
                        return resultAux
                ...
                if usuario:
                    ...
                    if usuario.id:  # si el usuario tiene id actualiza
                        ...
                        result = self.userPerfilData.get_usuario_perfil_by_id_usurio(
                            usuario.id, conexion
                        )
                        if not result["success"]:
                            conexion.rollback()
                        ...
                        if result["exists"] and id_per:
                            ...
                            result = self.userPerfilData.update_usuario_perfil(
                                result["usuarioPerfil"]["id"],
                                usuario.id,
                                id_per,
                                conexion,
                            )
                            ...
                            result = self.userPerfilData.delete_usuario_perfil(
                                result["usuarioPerfil"]["id"]
                            )
                            if not result["success"]:
                                conexion.rollback()
    ...
    def delete_Empleado(self, id_empleado: int):
        ...
        try:
            with conexion.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT Id_Persona FROM Empleado WHERE Id = %s;", (id_empleado,)
                )
                ...
                cursor.execute(
                    "SELECT Id FROM Usuario WHERE Id_Persona = %s;", (id_persona,)
                )
                resultado = cursor.fetchone()
                ...
                if resultado:
                    id_usuario = resultado[
                        0
                    ]  # Obtener el valor de Id_Usuario si existe

                # Si existe un usuario, eliminar registros dependientes
                if id_usuario:
                    # Eliminar registros en Mantenimiento
                    cursor.execute(
                        "DELETE FROM Mantenimiento WHERE Id_usuario = %s;",
                        (id_usuario,),
                    )

                    # Eliminar registros en Usuario_Perfil
                    cursor.execute(
                        "DELETE FROM Usuario_Perfil WHERE Id_Usuario = %s;",
                        (id_usuario,),
                    )
                ...
                cursor.execute(
                    "DELETE FROM Telefono WHERE Id_Persona = %s;", (id_persona,)
                )

                # Eliminar registros en Huella
                cursor.execute(
                    "DELETE FROM Huella WHERE Id_Empleado = %s;", (id_empleado,)
                )

                # Eliminar registros en Justificacion
                cursor.execute(
                    "DELETE FROM Justificacion WHERE Id_Empleado = %s;", (id_empleado,)
                )

                # Eliminar registros en Solicitud_Permiso
                cursor.execute(
                    "DELETE FROM Solicitud_Permiso WHERE Id_Empleado = %s;",
                    (id_empleado,),
                )

                # Eliminar registros en Reporte
                cursor.execute(
                    "DELETE FROM Reporte WHERE Id_Empleado = %s;", (id_empleado,)
                )
                ...
                cursor.execute(
                    "DELETE FROM Asistencia WHERE Id_Empleado = %s;", (id_empleado,)
                )

                # Eliminar registros en Empleado_Rol
                cursor.execute(
                    "DELETE FROM Empleado_Rol WHERE Id_Empleado = %s;", (id_empleado,)
                )
    ...
    def list_Empleados(
        ...
        try:
            with conexion.cursor(dictionary=True) as cursor:
                ...
                for data in registros:
                    ...
                    listaPersonas.append(
                        {"id_empleado": data["EMPLEADO_ID"], "persona": persona}
                    )
            ...
            return {
                "success": False,
                "message": "Ocurrió un error al listar los empleados.",
            }
        finally:
            if conexion:
                conexion.close()
    ...
    def getEmpleadoById(self, id_empleado: int, conexionEx=None):
        ...
        try:
            with conexion.cursor(dictionary=True) as cursor:
                ...
                if data:
                    ...
                    if not result["exists"]:
                        return {
                            "success": False,
                            "message": "No se obtuvieron los datos personales exitosamente",
                        }
                    persona = result["persona"]

                    # traer telefonos
                    result = self.telefonoData.get_Telefono_by_id_persona(
                        persona.id, conexion
                    )
                    if not result["success"]:
                        return result
                    ...
                    result = self.usuariodata.get_usuario_by_id(
                        data[TBEMPLEADO_PERSONA], conexion
                    )
                    if not result["success"]:
                        return result
            ...
            return {
                "success": False,
                "message": "Ocurrio un problema al obtener el empleado.",
            }
        finally:
            if conexion and conexionEx is None:
                conexion.close()
    ...
    def getAll_info_empleado_by_id(self, id_empleado: int, conexionEx):
        ...
        try:
            ...
            return {
                "success": False,
                "message": "Ocurrio un problema al obtener la informacion del empleado.",
            }
        finally:
