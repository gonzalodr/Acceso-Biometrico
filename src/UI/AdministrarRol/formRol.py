...
class formRol(QDialog):
    ...
    def __init__(self, parent=None, titulo="Registrar Rol", id=None):
        ...
        self.inputNombre.setPlaceholderText("Ingrese el nombre del rol")
        ...
        boton_box.accepted.connect(self._accion_rol)
    ...
    def _obtener_registroId(self, id):
        result = self.Rservices.obtenerRolPorId(id)
        if result["success"]:
            if result["data"]:
                rol = result["data"]
                self.idR = rol.id
                self.inputNombre.setText(rol.nombre)
                self.inputDescripcion.setText(rol.descripcion)
            else:
    ...
    def _accion_rol(self):
        ...
        if self._validar_campos():
            if self.idR > 0:
                result = self.Rservices.modificarRol(rol)
                if result["success"]:
                    dial = DialogoEmergente("Actualización", result["message"], "Check")
                    dial.exec()
                    self.reject()
                else:
                    dial = DialogoEmergente("Error", "Error al actualizar el rol", "Error")
                    dial.exec()
            else:
                result = self.Rservices.insertarRol(rol)
                if result["success"]:
                    dial = DialogoEmergente("Registrar", "Rol registrado exitosamente", "Check")
                    dial.exec()
                    self.reject()
                else:
                    dial = DialogoEmergente("Error", "Error al registrar el rol", "Error")
                    dial.exec()
