...
class AdminPermisosPerfil(QWidget):
    ...
    def __init__(self, parent= None) -> None:
        ...
        layoutTop.addWidget(self.crearBoton('Buscar',minimoTamBtn,self.buscarPerfiles))
        layoutTop.addStretch(1)
        ...
        header_labels = ["Perfil","Descripción","Acesso a", "Ver", "Crear", "Editar", "Eliminar","Acciones"]
    ...
    def cargarTablaPerfiles(self):
        result = self.perfilServices.obtenerListaPerfil(pagina=self.paginaActual, tam_pagina=10, tipo_orden="DESC", busqueda=self.busqueda)
        ...
        if len(listaPerfiles) == 0:
            self.mostrarMensajeTabla("No hay registros." if self.busqueda is None else "No se encontraron registros que coincidan con la busqueda.")  
            return 
        ...
        for index, perfiles in enumerate(listaPerfiles):
            ...
            if listPermisos:
                ...
                self.addItemTable(contadorPerfiles, 3, "Si" if primer_permiso.ver else "No")
                self.addItemTable(contadorPerfiles, 4, "Si" if primer_permiso.crear else "No")
                self.addItemTable(contadorPerfiles, 5, "Si" if primer_permiso.editar else "No")
                self.addItemTable(contadorPerfiles, 6, "Si" if primer_permiso.eliminar else "No")
                ...
                for permiso_idx, permiso in enumerate(listPermisos[1:], start=1):
                    ...
                    self.addItemTable(row_position, 6, "Si" if permiso.eliminar else "No")
                    self.addItemTable(row_position, 7, "")

                if len(listPermisos) > 1:
                    self.tbPermisosPerfil.setSpan(contadorPerfiles, 0, len(listPermisos), 1)  
                    self.tbPermisosPerfil.setSpan(contadorPerfiles, 1, len(listPermisos), 1)
                    self.tbPermisosPerfil.setSpan(contadorPerfiles, 7, len(listPermisos), 1)
                else:
                    self.tbPermisosPerfil.setRowHeight(contadorPerfiles,50)
            

            layout = QHBoxLayout()
            layout.addWidget(self.crearBtnAccion('Editar','btneditar',perfil.id,self.editarPerfil))
            layout.addSpacing(15)
            layout.addWidget(self.crearBtnAccion('Eliminar','btneliminar',perfil.id,self.eliminarPerfil))
    ...
    def crearBtnAccion(self,text:str,objectName:str,id_perfil:int,functionAction):
