...
class formPermiso(QDialog):
    ...
    def __init__(self, parent=None, titulo="Registrar permiso.", id=None):
        ...
        layoutForm.addLayout(
            self._contenedor(lblRol, self.inputRol, self.errorRol), 0, 0
        )
        layoutForm.addLayout(
            self._contenedor(lblAcceso, self.inputTabla, self.errorTabla), 1, 0
        )
    ...
    def _verificar_permiso(self):
        ...
        result = self.permisosServices.verificar_permiso_rol_tabla(
            rol_id=rolid, tabla=tabla, id=self.idP
        )

        if result and self.idP == 0:
            self.error.setText(
                f"El rol '{self.inputRol.currentText()}' ya posee el acceso a '{self.inputTabla.currentText()}'"
            )
            return False
        else:
            self.error.setText("")
    ...
    def _checked_verificacion(self):
        if (
            self.checkCrear.isChecked()
            or self.checkEditar.isChecked()
            or self.checkEliminar.isChecked()
        ):
    ...
    def _validar_campos(self):
        if self._validar_inputs_vacios():
            dialEmergente = DialogoEmergente(
                "Advertencia", "Debes conceder al menos un permiso.", "Warning"
            )
            dialEmergente.exec()
            return False
        else:
    ...
    def _validar_inputs_vacios(self):
        if (
            not self.checkCrear.isChecked()
            and not self.checkEditar.isChecked()
            and not self.checkEliminar.isChecked()
            and not self.checkVer.isChecked()
        ):
            return True
        else:
            return False
    ...
    def _obtener_registroId(self, id):
        ...
        if result["success"]:
            if result["data"]:
                ...
                rol = dato["data"]
                acceso_a = [
                    key for key, value in ACCESO_TABLE.items() if value == permiso.tabla
                ]
            ...
            dial = DialogoEmergente(
                "Error", "Hubo un error al intentar editar.", "Error"
            )
            dial.exec()
            QTimer.singleShot(0, self.reject)
            return None
        ...
        if self._validar_campos():
            if self.idP > 0:
                ...
                if result["success"]:
                    ...
                    dial = DialogoEmergente(
                        "Erro", "Error al actualizar a el permiso", "Error"
                    )
                    dial.exec()
            else:
                ...
                if result["success"]:
                    dial = DialogoEmergente(
                        "Registrar", "Permiso registrado exitosamente", "Check"
                    )
                    dial.exec()
                    self.reject()
                else:
                    dial = DialogoEmergente(
                        "Erro", "Error al registrar a el permiso", "Error"
                    )
                    dial.exec()
