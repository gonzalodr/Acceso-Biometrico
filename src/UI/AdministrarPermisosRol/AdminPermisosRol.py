...
class AdminPermisosRol(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.inputBuscar.setPlaceholderText(
            "Buscar persona por nombre, apellidos, cedula o correo."
        )
        ...
        self.tbPermisosRol.setHorizontalHeaderLabels(
            header_labels
    ...
    def _cargar_tabla(self):
        result = self.permisorolServices.listar_permisos_rol(
        ...
        if result["success"]:
            ...
            if len(listaPermisos) > 0:
                ...
                for index, permisos in enumerate(listaPermisos):
                    ...
                    acceso_a = [
                        key
                        for key, value in ACCESO_TABLE.items()
                        if value == permisos.tabla
                    ]
                    ...
                    self.addItem_a_tabla(
                        index, 5, ("Si" if permisos.eliminar else "No")
                    )

                    btnEliminar = QPushButton(text="Eliminar")
                    btnEliminar.clicked.connect(
                        lambda checked, idx=permisos.id: self._eliminarRegistro(idx)
                    )  ##cambier solo el persona.id por su objeto.id
                    ...
                    btnEditar.clicked.connect(
                        lambda checked, idx=permisos.id: self._editar_permiso(idx)
                    )  ##cambier solo el persona.id por su objeto, y el nombre de la funcion
