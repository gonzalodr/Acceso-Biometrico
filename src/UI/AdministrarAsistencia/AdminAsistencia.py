...
class AdminAsistencia(QWidget):
    ...
    ##acomodando botones de arriba en el layout
        ...
        self.tbAsistencia.horizontalHeader().setFixedHeight(40) # Establece una altura fija para los encabezados
        self.tbAsistencia.verticalHeader().setVisible(False)# Oculta los encabezados verticales
        self.tbAsistencia.horizontalHeader().setStretchLastSection(True) # Hace que la última sección de la tabla se estire
        self.tbAsistencia.setSelectionMode(QAbstractItemView.SelectionMode.NoSelection)# Deshabilita la selección de filas
    ...
    def _mostrar_mensaje_sin_datos(self):
        ...
        if self.tbAsistencia.rowCount() == 0: # Si no hay filas
            self.tbAsistencia.setRowCount(1)# Establecer una fila para mostrar el mensaje
            item = QTableWidgetItem("Sin datos")
            item.setTextAlignment(Qt.AlignCenter)
            self.tbAsistencia.setItem(0, 0, item)# Establecer el item en la primera celda
            for col in range(1, self.tbAsistencia.columnCount()):
                self.tbAsistencia.setItem(0, col, QTableWidgetItem(""))  # Celdas vacías
            self.tbAsistencia.setSpan(0, 0, 1, self.tbAsistencia.columnCount())# Hacer que el mensaje ocupe toda la f
            
            
    def _cargar_tabla(self):
          # Método para cargar los datos en la tabla
        result = self.asistenciaServices.obtenerListaAsistencia(pagina=self.paginaActual, tam_pagina=10, tipo_orden="DESC", busqueda=self.busqueda)
        if result["success"]:
            listaAsistencias = result["data"]["listaAsistencias"] # Obtener la lista de perfiles
 ...
 # Agregar los datos del perfil a la tabla
    ...
    def _editar_Asistencia(self, id_asistencia):
        ...
        asistencia_form = formAsistencia(id=id_asistencia)# Abrir el formulario de edición
        asistencia_form.exec()
        self._cargar_tabla()
        self._cargar_tabla()
