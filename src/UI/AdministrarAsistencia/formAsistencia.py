...
class formAsistencia(QDialog):
    ...
    def __init__(self, parent=None, titulo="Registrar asistencia.",id = None):
        ...
        layoutForm.addLayout(self._contenedor(lblEstado, self.inputEstado, self.errorEstado), 3, 0)
   ...
   # def _cancelar_registro(self):
    ...
    def _accion_asistencia(self):       
        ...
        if self.idP and self.idP > 0:  # Verifica que self.idP no sea None y sea mayor a 0
            result = self.asistenciaServices.modificarAsistencia(asistencia)
            mensaje = "Actualización" if result["success"] else "Error al actualizar el perfil"
        else:
            result = self.asistenciaServices.insertarAsistencia(asistencia)
            mensaje = "Reporte registrado exitosamente" if result["success"] else "Error al registrar el Reporte"

        icono = "Check" if result["success"] else "Error"
        dial = DialogoEmergente(mensaje, result["message"] if result["success"] else "Error", icono)
        dial.exec()
