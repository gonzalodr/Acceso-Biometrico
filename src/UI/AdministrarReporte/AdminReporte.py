...
class AdminReporte(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.tbReporte.horizontalHeader().setFixedHeight(
            40
        )  # Establece una altura fija para los encabezados
        self.tbReporte.verticalHeader().setVisible(
            False
        )  # Oculta los encabezados verticales
        self.tbReporte.horizontalHeader().setStretchLastSection(
            True
        )  # Hace que la última sección de la tabla se estire
        self.tbReporte.setSelectionMode(
    ...
    def _mostrar_mensaje_sin_datos(self):
        ...
        if self.tbReporte.rowCount() == 0:  # Si no hay filas
            ...
            self.tbReporte.setSpan(
                0, 0, 1, self.tbReporte.columnCount()
            )  # Hacer que el mensaje ocupe toda la f

    def _cargar_tabla(self):
        # Método para cargar los datos en la tabla

        result = self.reporteServices.obtenerListaReporte(
        ...
        if result["success"]:
            ...
            if len(listaReporte) > 0:
                ...
                for reporte in listaReporte:
                    print(
                        f"- Nombre: {reporte['nombre_empleado']}, Fecha: {reporte['reporte'].fecha_generacion}, Tipo: {reporte['reporte'].tipo_reporte}, Contenido: {reporte['reporte'].contenido}"
                    )

                for index, reporte in enumerate(
                    listaReporte
                ):  # Iterar sobre la lista de perfiles
                    self.tbReporte.insertRow(
                        index
                    )  # Insertar una nueva fila en la tabla
                    self.tbReporte.setRowHeight(
                        index, 45
                    ...
                    self.addItem_a_tabla(
                        index,
                        1,
                        reporte["reporte"].fecha_generacion.strftime("%Y-%m-%d"),
                    )
                    ...
                    btnEditar.clicked.connect(
                        lambda checked, idx=reporte["reporte"].id: self._editar_Reporte(
                            idx
                        )
                    )
