...
class formReporte(QDialog):
    ...
    def __init__(self, parent=None, titulo="Registrar reporte.", id=None):
        ...
        self.inputTipo.addItems(
            ["Incidencia", "Informe", "Solicitud", "Otro"]
        )  # Tipos de reportes
        ...
        layoutForm.addLayout(
            self._contenedor(lblTipo, self.inputTipo, self.errorTipo), 3, 0
        )
    ...
    def _obtener_registroId(self, id):
        ...
        if result["success"]:
            if result["data"]:
                ...
                fecha_qt = QDate(
                    reporte.fecha_generacion.year,
                    reporte.fecha_generacion.month,
                    reporte.fecha_generacion.day,
                )
        ...
        if self.idP > 0:
            ...
            if result["success"]:
                ...
                dial = DialogoEmergente(
                    "Error", "Error al actualizar el perfil", "Error"
                )
                dial.exec()
        else:
            ...
            if result["success"]:
                dial = DialogoEmergente(
                    "Registrar", "Reporte registrado exitosamente", "Check"
                )
                dial.exec()
                self.reject()
            else:
                dial = DialogoEmergente(
                    "Error", "Error al registrar el Reporte", "Error"
                )
                dial.exec()
